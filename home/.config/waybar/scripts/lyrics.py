...
def out(text="", tooltip="", cls="none"):
    print(json.dumps({"text": text, "tooltip": tooltip, "class": cls, "alt": cls}), flush=True)
    raise Done()
...
# persistent: waybar reads one JSON line per second instead of spawning a new interpreter each time
import time
last = None
while True:
    try:
        ...
        sys.exit(0)
    except Exception as e:  # never die: report and keep going
        print(json.dumps({"text": "", "tooltip": f"lyrics: {e}", "class": "none"}), flush=True)
    time.sleep(1)
